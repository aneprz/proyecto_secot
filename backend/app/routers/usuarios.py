...
def list_usuarios(
    ...
    sql = f"""
        select usuario_id, username, email, rol, senior_id, activo, creado_en, ultimo_login_en
        from usuario
        {where}
...
def create_usuario(payload: UsuarioCreate):
    ...
    sql = """
        insert into usuario (username, email, password_hash, rol, senior_id, activo)
        values (%(username)s, %(email)s, %(password_hash)s, %(rol)s, %(senior_id)s, %(activo)s)
        returning usuario_id, username, email, rol, senior_id, activo, creado_en, ultimo_login_en;
    """
    try:
        with get_connection(row_factory=dict_row) as conn:
            with conn.cursor() as cur:
