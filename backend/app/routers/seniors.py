...
def list_seniors(
    ...
    sql = f"""
        select senior_id, nombre, apellido1, apellido2, email_personal, email_secot, movil, fecha_alta, activo
        from senior
        {where}
...
def create_senior(payload: SeniorCreate, _: str = Depends(require_write)):
    sql = """
        insert into senior (nombre, apellido1, apellido2, email_personal, email_secot, movil, fecha_alta, activo)
        values (%(nombre)s, %(apellido1)s, %(apellido2)s, %(email_personal)s, %(email_secot)s, %(movil)s, %(fecha_alta)s, %(activo)s)
        returning senior_id, nombre, apellido1, apellido2, email_personal, email_secot, movil, fecha_alta, activo;
    """
    try:
        with get_connection(row_factory=dict_row) as conn:
            with conn.cursor() as cur:
        ...
        message = str(exc)
        if (
            "uq_senior_email_personal" in message
            or "uq_senior_email_secot" in message
