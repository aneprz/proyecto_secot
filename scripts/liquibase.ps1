param(
  ...
  [ValidateSet('update', 'status', 'validate', 'history', 'tag', 'rollbackCount')]
...
if (-not $jdbcUrl) {
  ...
  if ($projectRef -and $dbHost -and $dbHost -like '*.pooler.supabase.com' -and $dbUser -and ($dbUser -notmatch '\.')) {
    ...
    Write-Host "Info: host de pooler detectado. Usando usuario '$dbUser' (formato requerido por Supabase pooler)."
  }

  if ($dbHost) {
    try {
      $resolved = [System.Net.Dns]::GetHostAddresses($dbHost)
      $hasIpv4 = @($resolved | Where-Object { $_.AddressFamily -eq [System.Net.Sockets.AddressFamily]::InterNetwork }).Count -gt 0
      $hasIpv6 = @($resolved | Where-Object { $_.AddressFamily -eq [System.Net.Sockets.AddressFamily]::InterNetworkV6 }).Count -gt 0
      if (-not $hasIpv4 -and $hasIpv6) {
        Write-Host "Aviso: $dbHost solo resuelve a IPv6. Si Docker no tiene IPv6, usa el host/puerto del 'Connection Pooler' de Supabase."
      }
    } catch {
      Write-Host "No se puede resolver el host DNS: $dbHost"
      Write-Host "Esto suele indicar que el host no es el correcto."
      Write-Host "Ve a Supabase -> Project Settings -> Database -> Connection info y copia el 'Host' real."
      Write-Host "Luego ponlo en SUPABASE_DB_HOST (y revisa SUPABASE_DB_PORT: 5432 directo, 6543/6432 pooler)."
      exit 1
    }
...
if (-not $jdbcUrl -or -not $dbUser -or -not $dbPass) {
  ...
  Write-Host "Crea un .env (o credenciales.env) a partir de .env.example y rellena:"
  Write-Host "  SUPABASE_DB_HOST (o SUPABASE_DB_JDBC_URL)"
  Write-Host "  SUPABASE_DB_USER"
  Write-Host "  SUPABASE_DB_PASSWORD"
  exit 1
}

$docker = Get-Command docker -ErrorAction SilentlyContinue
if (-not $docker) {
  Write-Host "No encuentro 'docker'. Instala Docker Desktop (recomendado) o Liquibase CLI + driver PostgreSQL."
  exit 1
}
...
if (-not $postgresDriver) {
  ...
  Write-Host "Descarga el archivo postgresql-*.jar y colócalo en: db/drivers/"
  Write-Host "Ejemplo (PowerShell):"
  Write-Host "  Invoke-WebRequest -OutFile db/drivers/postgresql.jar https://jdbc.postgresql.org/download/postgresql-42.7.7.jar"
  exit 1
}
...
switch ($Command) {
  'tag' {
    if (-not $Arg1) { throw "Uso: scripts/liquibase.ps1 tag <nombre-tag>" }
    & docker @dockerArgs "--classpath=$classpath" 'tag' $Arg1 @Rest
  }
  'rollbackCount' {
    if (-not $Arg1) { throw "Uso: scripts/liquibase.ps1 rollbackCount <n>" }
    & docker @dockerArgs "--classpath=$classpath" 'rollbackCount' $Arg1 @Rest
  }
  default {
    & docker @dockerArgs "--classpath=$classpath" $Command @Rest
  }
}
